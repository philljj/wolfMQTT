...
static int multirecv_test_init(MQTTCtx *mqttCtx)
{
...
static int multirecv_test_finish(MQTTCtx *mqttCtx)
{
    client_disconnect(mqttCtx);
...
int multirecv_test(MQTTCtx *mqttCtx)
{
    ...
    rc = multirecv_test_init(mqttCtx);
    if (rc == 0) {
        ...
        rc = multirecv_test_finish(mqttCtx);
    }
    return rc;
}
...
#endif
{
    ...
    gMqttCtx.app_name = "wolfMQTT multirecv client";
